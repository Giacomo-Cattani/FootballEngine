...
function substitutePlayer(matchDetails, team, player) {
  let subIndex = team.subs.findIndex(function (thisPlayer) {
    return thisPlayer.playerID === player.playerID
  })
  let playerIndex = team.players.findIndex(function (thisPlayer) {
    return thisPlayer.playerID === player.playerID
  })
  if (subIndex) {
    team.players.push(team.subs[subIndex])
    team.subs.push(team.players[playerIndex])
    team.player.splice(playerIndex, 1)
    team.subs.splice(subIndex, 1)
    matchDetails.iterationLog.push(`Substitution: ${player.name} replaced by ${team.subs[subIndex].name}`)
  }
}
